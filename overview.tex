...
We envision the framework consisting of three modular components:
(1) a Time Budget Estimator deployed at edge servers that estimates per-request time budgets using passive observation of network timing patterns,
(2) an Edge Resource Manager that implements deadline-aware scheduling for CPU and GPU resources along with admission control and early-drop policies, and
(3) a Policy Engine that enables operators to configure service tiers and fairness policies without modifying application code.
